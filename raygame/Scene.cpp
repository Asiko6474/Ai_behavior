...
void Scene::update(float deltaTime)
{
    ...
    for (int i = 0; i < m_actors.getLength(); i++)
    {
        if (!m_actors.get(i)->getStarted())
            m_actors.get(i)->start();

        m_actors.get(i)->update(deltaTime);
    }
    ...
    for (int i = 0; i < m_actors.getLength(); i++)
    {
        ...
        {
            if (m_actors.get(i)->checkForCollision(m_actors.get(j)) && j != i && m_actors.get(j)->getStarted())
                m_actors.get(i)->onCollision(m_actors.get(j));
        }
    }
}
...
void Scene::updateUI(float deltaTime)
{
    ...
    {
        if (!m_UIElements.get(i)->getStarted())
            m_UIElements.get(i)->start();
